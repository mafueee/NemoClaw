...
function setCredential(providerKey, apiKey, endpointUrl = null) {
  ...
  if (provider && provider.providerName && apiKey) {
    try {
      const type = provider.type;
      const credParam = `${provider.credentialEnv || 'API_KEY'}=${apiKey}`;
      let endpointParam = "";
      const finalEndpoint = endpointUrl || provider.endpoint;
      
      if (finalEndpoint && (provider.custom || provider.endpoint !== finalEndpoint)) {
        endpointParam = `--config endpoint=${finalEndpoint} `;
      }

      runCapture(`openshell provider delete ${provider.providerName}`, { ignoreError: true });
      runCapture(`openshell provider create --name ${provider.providerName} --type ${type} ${endpointParam}--credential "${credParam}"`, { ignoreError: true });
    } catch (err) {
      console.warn("Failed to push credential to gateway:", err.message);
    }
  }
}
...
function getEndpoint(providerKey) {
  const creds = loadCredentials();
  return creds[providerKey]?.endpointUrl || null;
}
...
/**
 * Switch the active inference model on a running sandbox.
 *
 * @param {string} providerName - Provider identifier (e.g., "nvidia-prod")
 * @param {string} model - Model name (e.g., "nvidia/nemotron-3-super-120b-a12b")
 ...
 *
 * @returns {{ provider: string, model: string, endpoint: string } | null}
 */
...
/**
 * Validate a provider/model combination by sending a test request.
 *
 * @param {string} providerKey - Key from the PROVIDERS catalogue
...
function validateProvider(providerKey, endpoint, apiKey, model) {
  ...
  // For cloud providers, try validation endpoint
