...
/**
 * Idempotent provider upsert.  Handles the gateway's restriction that
 * provider type cannot be changed via updateProvider.
 *
 * Strategy:
 *  1. Try createProvider — success → done.
 *  2. ALREADY_EXISTS (code 6) → getProvider to check existing type.
 *     a. Same type → updateProvider (credentials/config only, no type).
 *     b. Different type → deleteProvider, then createProvider.
 *
 * @param {string} name        Gateway provider name (e.g. 'nemoclaw-openrouter')
 * @param {string} type        Gateway-compatible type ('openai'|'anthropic'|'nvidia')
 * @param {object} credentials { api_key: '...' }
 * @param {object} config      { base_url: '...' }
 */
export async function ensureProvider(name, type, credentials = {}, config = {}) {
    try {
        await createProvider({ name, type, credentials, config });
        return;
    } catch (createErr) {
    ...
    // Provider already exists — check if the type matches
    let existing;
    try {
        existing = await getProvider(name);
    } catch {
        // Cannot fetch existing provider; try delete + recreate as fallback
        await deleteProvider(name);
        await createProvider({ name, type, credentials, config });
        return;
    }

    const existingType = existing?.provider?.type || existing?.type || '';
    if (existingType === type) {
        // Same type — safe to update (omit type field to avoid gateway rejection)
        await updateProvider({ name, credentials, config });
    } else {
        // Type changed — must delete and recreate
        await deleteProvider(name);
        await createProvider({ name, type, credentials, config });
    }
}
...
/**
 * Idempotent sandbox creation.  If a sandbox with the same name already
 * exists (UNIQUE constraint, gRPC code 13 INTERNAL), delete it first and
 * recreate.
 *
 * @param {object} spec     Sandbox spec (environment, template, policy, providers)
 * @param {string} name     Sandbox name
 * @returns {object}        createSandbox response { sandbox }
 */
export async function ensureSandbox(spec, name) {
    try {
        return await createSandbox(spec, name);
    } catch (err) {
        // Check for UNIQUE constraint violation
        const isUnique = (err.code === 13 /* INTERNAL */ && err.message?.includes('UNIQUE constraint'))
    ...
    // Sandbox already exists — delete it and recreate
