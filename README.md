...
| **Agent chat** | `ExecSandbox` (server-streaming) | Runs `openclaw chat` inside the sandbox via gRPC — all inference is policy-constrained. Falls back to direct LLM proxy if OpenClaw is missing. |
...
| **Agent Chat** | Web-based chat routed through the sandbox via `ExecSandbox` gRPC — agent actions are constrained by the claw's security policy. Automatic fallback to direct LLM proxy with warning if OpenClaw is not installed. |
...
- Adding **agent chat** routed through `ExecSandbox` gRPC for policy-constrained inference
