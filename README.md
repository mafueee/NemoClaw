...
- **`lib/grpcClient.js`** — Persistent gRPC channel with mTLS credential loading, connection pooling, typed async wrappers for all OpenShell and Inference RPCs, **provider type mapping** (`mapProviderToGrpcType`), and **idempotent deployment helpers** (`ensureProvider` handles type conflicts via delete+recreate; `ensureSandbox` handles UNIQUE constraint violations).
